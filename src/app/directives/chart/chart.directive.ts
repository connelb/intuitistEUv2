...
export class ChartDirective implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    // const reduceFn = data => d3.sum(data, d => d['score']);
    ...
    // this.hierarchyData = d3.hierarchy([null, rollupData], childrenAccessorFn)
    //   .sum(([key, value]) => value)
  ...
  createChart2() {
    ...
    function bar(svg, down, d, selector) {
      ...
      const bar = g.selectAll("g")
        ...
        .on("click", d => !d.children ? console.log('child:',d.data.name) : down(svg, d));
